...
export default function User() {
  ...
  const me = useAuthStore((state) => state.userData);
      ...
      {info && (
        <div className="w-screen h-[88vh] px-[3vw] md:px-[10vw] py-10 flex">
          <div className="hidden md:flex md:w-[30%] flex-col">
            {info.usercode === me?.usercode ? (
              <span className="text-2xl font-yapari text-[var(--primary-300)]">
                MY
              </span>
            ) : (
              <span className="text-2xl font-yapari text-white">
                {info.username}
              </span>
            )}
            <span className="text-2xl font-yapari text-[var(--primary-300)]">
          ...
          <div className="flex flex-col gap-8 w-full md:w-[70%]">
            <div className="flex gap-2 md:gap-6 items-center">
              <img
                src={info.avatar_url || defaultImg}
                alt="profileImage"
                className="rounded-full size-22 aspect-square object-cover object-center"
              />
            ...
            <div className="min-h-15 mt-2">
              <div className=" border-l border-[var(--primary-300)] pl-5">
                {info.bio ? info.bio : "채널 소개문이 없습니다."}
              </div>
            </div>
